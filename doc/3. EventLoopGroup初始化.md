...
// TODO: MultithreadEventLoopGroup 子类间的区别
...
SelectorProvider.provider()为创建由操作系统提供的用于select方式的NIO对象

RejectedExecutionHandlers.reject() 获得一个类似于java.util.concurrent.RejectedExecutionHandler的拒绝处理器，但限定于SingleThreadEventExecutor
...
在这里判断了线程数是否为0，为0则赋值为CPU核心数的两倍。即：
...
最后创建了一个NioEventLoop对象，此时的taskQueueFactory和tailTaskQueueFactory均为null。继续深入NioEventLoop#NioEventLoop的调用链
...
// NioEventLoop#newTaskQueue(io.netty.channel.EventLoopTaskQueueFactory)
...
DEFAULT_MAX_PENDING_TASKS在没有指定时为Integer.MAX_VALUE，并且当前taskQueueFactory和tailTaskQueueFactory均为null，所以两个newTaskQueue方法均会从newTaskQueue0进入newMpscQueue()方法来创建[Mpsc Queue](https://www.cnblogs.com/crazymakercircle/p/16977089.html)。

Mpsc 为Multi Producer Single Consumer，即多生产者单消费者，可以保证多个生产者同时访问队列是线程安全的，而且同一时刻只允许一个消费者从队列中读取数据，此处不再展开。

首先会调用父类构造函数SingleThreadEventLoop#SingleThreadEventLoop
...
再进到SingleThreadEventExecutor#SingleThreadEventExecutor
...
// AbstractScheduledEventExecutor#AbstractScheduledEventExecutor(io.netty.util.concurrent.EventExecutorGroup)
protected AbstractScheduledEventExecutor(EventExecutorGroup parent) {
    super(parent);
}
// AbstractEventExecutor#AbstractEventExecutor(io.netty.util.concurrent.EventExecutorGroup)
...
这部分做了两个操作，一方面初始化this.executor为一个使用传入参数executor执行任务的代理，在真正执行提交的任务前先执行apply方法，另一方面通过apply方法将每个提交到this.executor的任务使用匿名内部类来进行代理执行。

```java
// ThreadExecutorMap#apply(java.util.concurrent.Executor, io.netty.util.concurrent.EventExecutor)
...
// ThreadExecutorMap#apply(java.lang.Runnable, io.netty.util.concurrent.EventExecutor)
...
最后在AbstractEventExecutor构造器中初始化了parent，也就是从NioEventLoop(this, executor,  ...);传过来的bossGroup。
...
children的长度，也就是线程数是否是2的幂来选择EventExecutorChooser：
...
> 之所以根据executors.length来选择selector，是因为2的幂的长度-1全为1，如8-1 -> 1000 - 1 = 0111，这样能够将所有位均选择，而不为2的幂的长度如 6，6-1 -> 0110 - 1 = 0101，会存在丢失，导致部分children永远不会被选择。
...
在使用`children[i] = newChild(executor, args);`创建完NioEventLoop之后没有直接调用启动函数，线程是否还未启动？

在SingleThreadEventExecutor#execute(java.lang.Runnable, boolean)打上断点后debug发现调用栈为：

![NioEventLoopCallStack](.assert/NioEventLoopCallStack.png)

可以发现，该方法最初是由第2节中提到的`serverBootstrap.bind(10009)`方法的调用栈。在[第2节](./2. 启动服务前ServerBootstrap类型如何初始化.md)中已经提到过bind的流程，现在直接看SingleThreadEventExecutor#execute(java.lang.Runnable)的执行过程。
...
// SingleThreadEventExecutor#wakesUpForTask
// 用于判断任务是否为即时任务
...
保证了同一个channel的注册、读、写等都在eventLoop完成，避免多线程的锁竞争。
